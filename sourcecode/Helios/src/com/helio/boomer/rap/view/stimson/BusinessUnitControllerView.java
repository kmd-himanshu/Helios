...
public class BusinessUnitControllerView extends ViewPart {

	public static final String ID = "com.helio.boomer.rap.view.stimson.businessunitcontrollerview";
	...
	public BusinessUnitControllerView() {
	...
	public void createPartControl(Composite parent) {
		...
		lblStartPeriod.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1));
		...
		lblEndPeriod.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1));
		...
		btnRecalcCasesHandled.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				Calendar begCalendar = new GregorianCalendar(dateTimeStart
						.getYear(), dateTimeStart.getMonth(), dateTimeStart
						.getDay());
				Date begDate = new Date(begCalendar.getTimeInMillis());
				Calendar endCalendar = new GregorianCalendar(dateTimeEnd
						.getYear(), dateTimeEnd.getMonth(), dateTimeEnd
						.getDay());
				Date endDate = new Date(endCalendar.getTimeInMillis());
				try {
					List<Period> linkedPeriodList = new LinkedList<Period>();
					Period onPeriod = (Period) ((IStructuredSelection) startPeriodComboViewer
							.getSelection()).getFirstElement();				
					
					Period lastPeriod = (Period) ((IStructuredSelection) endPeriodComboViewer
							.getSelection()).getFirstElement();
					...
					List<Long> buildingIds = businessUnitNavView
							.getSelectedBuildingIds();
					List<Long> locationIds = businessUnitNavView
				 ...
				 */
					synchronized(this)
					{
						...
						BusinessUnitDataStore businessUnitDataStore = new BusinessUnitDataStore();
						
						// Distribution (LOC)
						businessUnitDataStore.setOnPeriod(onPeriod);
						businessUnitDataStore.setLastPeriod(lastPeriod);						
						businessUnitDataStore.setBeginDay(dateTimeStart.getDay());
						businessUnitDataStore.setBeginMonth(dateTimeStart.getMonth());
						businessUnitDataStore.setBeginYear(dateTimeStart.getYear());						
						businessUnitDataStore.setEndDay(dateTimeEnd.getDay());
						businessUnitDataStore.setEndMonth(dateTimeEnd.getMonth());
						businessUnitDataStore.setEndYear(dateTimeEnd.getYear());						
						businessUnitDataStore.setSelectedPeriodBegin(startPeriodComboViewer.getSelection());
						businessUnitDataStore.setSelectedPeriodEnd(endPeriodComboViewer.getSelection());						
						
						// Business Unit Navigation
						businessUnitDataStore.setListBuilding(buildingIds);
						businessUnitDataStore.setListLocation(locationIds);
						businessUnitDataStore.setExpandedElements(businessUnitNavView.getExpandedList());
						businessUnitDataStore.setSelectedElement(businessUnitNavView.getSelectedElements());
						
						// Graph Control
						businessUnitDataStore.setShowLegend(businessUnitDashboardView.getShowLegends());
						businessUnitDataStore.setShowAxesLabels(businessUnitDashboardView.getShowAxesLabels());
						businessUnitDataStore.setShowTitles(businessUnitDashboardView.getShowTitles());	
						
						/*
						 * RMAP:74 :
						 * The update methods have been modified.
						 * a>The return type for them is changed to businessUnitDataStore
						 * b> Additional parameter businessUnitDataStore is passed in method
						 * c>These method has been included in synchronized block
						 * d>businessUnitDataStore is then stored in session for returning view
						 */
					
					if ((buildingIds != null) && (buildingIds.size() > 0)) {
						try {
							processWarningMessage("API Information",
									"About to update Energy Cost per Square Foot Chart");
							businessUnitDataStore = businessUnitDashboardView
									.updateEnergyCostPerSqFtChart(buildingIds,
											linkedPeriodList,
											includeLocationName,businessUnitDataStore);
						} catch (Exception ex1) {
						...
						try {
							processWarningMessage("API Information",
									"About to update Energy per Business Unit Chart");
							businessUnitDataStore = businessUnitDashboardView.updateEnergyPerBUChart(
									buildingIds, begDate, endDate,
									includeLocationName,businessUnitDataStore);
						} catch (Exception ex1) {
							processWarningMessage("API Error", ex1.toString());
							System.out.println(ex1.getMessage());
						...
						try {
							...
							businessUnitDataStore = businessUnitDashboardView
									.updateDemandPerLocationChart(locationIds,
											begDate, endDate,businessUnitDataStore);
						} catch (Exception ex1) {
							processWarningMessage("API Error", ex1.toString());
							System.out.println(ex1.getMessage());
						}
						try {
							...
							businessUnitDataStore = businessUnitDashboardView
									.updateEnergyCasesHandledChart(locationIds,
											linkedPeriodList,businessUnitDataStore);
						} catch (Exception ex1) {
							processWarningMessage("API Error", ex1.toString());
							System.out.println(ex1.getMessage());
						}
					...
					sessionDataStore.setBusinessDataStore(businessUnitDataStore);						
					RWT.getSessionStore().setAttribute(sessionId, sessionDataStore);
		...
		ScrolledComposite graphControlScrolledComposite = new ScrolledComposite(
				graphControlShelfItem.getBody(), SWT.BORDER | SWT.H_SCROLL
						| SWT.V_SCROLL);
		graphControlScrolledComposite.setExpandHorizontal(true);
		graphControlScrolledComposite.setExpandVertical(true);
		Composite graphControlComposite = new Composite(
				graphControlScrolledComposite, SWT.NONE);
		graphControlComposite.setLayout(new GridLayout(2, false));
		graphControlScrolledComposite.setContent(graphControlComposite);
		graphControlScrolledComposite.setMinSize(graphControlComposite
		...
		legendButton.setSelection(businessUnitDashboardView.getShowLegends());		
		RWT.getSessionStore().setAttribute("legendButton",
				legendButton.getSelection());
		legendButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				businessUnitDashboardView.setShowLegends(legendButton
						.getSelection());
			}
		});
		...
		axesButton.setSelection(businessUnitDashboardView.getShowAxesLabels());
		RWT.getSessionStore().setAttribute("axesButton",
				axesButton.getSelection());
		axesButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				businessUnitDashboardView.setShowAxesLabels(axesButton
						.getSelection());
			}
		});
		...
		titleButton.setSelection(businessUnitDashboardView.getShowTitles());
		RWT.getSessionStore().setAttribute("titleButton",
				titleButton.getSelection());
		titleButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				businessUnitDashboardView.setShowTitles(titleButton
						.getSelection());
		...
		// ScrolledComposite mfgScrolledComposite = new
		// ScrolledComposite(manufacturingShelfItem.getBody(), SWT.BORDER |
		// SWT.H_SCROLL | SWT.V_SCROLL);
		// mfgScrolledComposite.setExpandHorizontal(true);
		// mfgScrolledComposite.setExpandVertical(true);

		Composite mfgComposite = new Composite(
				manufacturingShelfItem.getBody(), SWT.BORDER);
		...
		// mfgScrolledComposite.setMinSize(mfgComposite.computeSize(SWT.DEFAULT,
		// SWT.DEFAULT));
		...
		synchronized(this)
		{
			...
			BusinessUnitDataStore businessUnitDataStoreAppl=null;
			
			if(sessionDataStoreAppl != null)
			{
				businessUnitDataStoreAppl = sessionDataStoreAppl.getBusinessDataStore();
				if(businessUnitDataStoreAppl != null && sessionDataStoreLocal.isFirstScreenBusiness())
				{					
					populateLastLoginValues(businessUnitDataStoreAppl);			
				}
			}
			sessionDataStoreLocal.setFirstScreenBusiness(false);
	...
	private ComboViewer getStartPeriodComboViewer(Composite parent) {
		...
		GridData gd_periodCombo = new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 1, 1);
		...
		periodComboViewer.setInput(PeriodListController.getInstance()
				.getPeriodModelListAsArray());
		periodComboViewer
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (ss.getFirstElement() instanceof Period) {
							...
							dateTimeStart.setDate(startDt.get(Calendar.YEAR),
									startDt.get(Calendar.MONTH),
									startDt.get(Calendar.DAY_OF_MONTH));
				...
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (ss.getFirstElement() instanceof Period) {
							...
							Calendar endDt = new GregorianCalendar();
							endDt.setTime(period.getEndDt());
							dateTimeEnd.setDate(endDt.get(Calendar.YEAR),
									endDt.get(Calendar.MONTH),
									endDt.get(Calendar.DAY_OF_MONTH));
