...
public class EnergyPerCasesHandledPerBuildingChart extends HelioChart {

	public EnergyPerCasesHandledPerBuildingChart(ChartModel chartModel)
	{
		if (chartModel.getBuildingIds() != null
			...
			try {
				...
				currentValueMap = DistributionReportDAO.getEnergyCHPerBuilding(chartModel.getBuildingIds(),chartModel.getStartSqlDate(),chartModel.getEndSqlDate());
		...
		if (valueMap == null || valueMap.keySet().size()==0) {
		 ...
		 chartModel.setChartTitleLabel("Energy per Cases Handled per Building");		
