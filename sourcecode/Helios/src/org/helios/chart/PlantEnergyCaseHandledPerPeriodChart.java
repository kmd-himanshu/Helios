...
public class PlantEnergyCaseHandledPerPeriodChart extends HelioChart {

	public PlantEnergyCaseHandledPerPeriodChart(ChartModel chartModel)
	{
		if (chartModel.getBuildingIds() != null
				&& chartModel.getBuildingIds().size() > 0) {
			
			try {
				currentValueMap = DistributionReportDAO.getPrimaryCasesHandledKWhForLocation(
